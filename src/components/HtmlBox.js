...
import styles from "../styles/HtmlBox.module.css";
...
const HtmlBox = ({position, rotationy, rotationx, rotationz, color, onChangeColor, onChangeSlider, zoom}) => {
  ...
  return (
    <a.mesh  position={position}  rotation-y={rotationy}  rotation-x={rotationx}  rotation-z={rotationz}>
      ...
      <a.mesh>
        <Html className={styles.content} rotation-x={-Math.PI / 2} position={[0, 2.001, 0]} transform occlude>
          <div  className={styles.wrapper}>
            <img src={slide1} alt="Logo" width="160" height="160" />
          </div>
        </Html>
    
        <Html className={styles.content} rotation-y={Math.PI / 2} rotation-z={-Math.PI / 2} position={[2.001, 0, 0]} transform occlude>
          <div className={styles.wrapper}>
              <img src={slide2} alt="Logo" width="160" height="160" />
          </div>
        </Html>
         <Html className={styles.content} rotation-x={Math.PI / 2} rotation-z={Math.PI / 1}  position={[0, -2.001, 0]} transform occlude>
          <div className={styles.wrapper}>
              <img src={slide3} alt="Logo" width="160" height="160" />
          </div>
        </Html>
           <Html className={styles.content} rotation-x={-Math.PI / 2} rotation-y={-Math.PI / 2} position={[-2.001, 0, 0]} transform occlude>
          <div className={styles.wrapper}>
          <img src={slide4} alt="Logo" width="160" height="160" />
          </div>
        </Html>
        <Html className={styles.content} rotation-z={Math.PI / 2} rotation-x={Math.PI / 1}  position={[0, 0, -2.001]} transform occlude>
          <div className={styles.wrapper5}>
             <img src={slide5top} alt="Logo" width="160" height="96" />
             <div className={styles.slidersContainer}>
             <input className={styles.color}  onChange={(e) => onChangeColor(e.target.value)} type="color" id="color" name="color"
              value={color}></input>
        ...
        <Html className={styles.content} rotation-z={Math.PI / 2}  position={[0, 0, 2.001]} transform occlude>
          <div className={styles.wrapper}>
